...
public class InstructorDAOImpl implements InstructorDAO{
    ...
    @Override
    @Transactional
    public void update(Student student) {entityManager.merge(student);}//Based on the id, if id 0 was passed, will add it, if the id already exits will patch it
    ...
    public Course findCourseByIdWithStudents(int courseId) {
        return entityManager.createQuery("SELECT c FROM Course c JOIN FETCH c.students WHERE c.id = :courseId", Course.class)
                .setParameter("courseId", courseId).getSingleResult();
    }

    @Override
    public Student findStudentByIdWithCourses(int studentId) {
        return entityManager.createQuery("SELECT s FROM Student s JOIN FETCH s.courses WHERE s.id = :studentId", Student.class)
